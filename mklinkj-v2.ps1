...
function Copy-DirectoryWithProgress {
    ...
    try {
        ...
        if ($errors.Count -gt 0) {
            ...
            [System.Windows.Forms.MessageBox]::Show($errorMsg, "复制错误", 
...
$labelDescription.Text = "此工具用于创建目录联接（Junction Point），等于mklink /j命令。"
...
$checkBoxOpenAfter.Text = "创建后打开源目录"
...
$checkBoxOpenAfter.Checked = $false
...
$buttonTest.Add_Click({
    ...
    if (($source -eq "") -or ($target -eq "")) {
        [System.Windows.Forms.MessageBox]::Show("请输入源目录和目标目录", "错误", 
            [System.Windows.Forms.MessageBoxButtons]::OK, 
            [System.Windows.Forms.MessageBoxIcon]::Warning)
        return
    }
    ...
    if (Test-Path $source) {
        try {
            ...
            if ($attr -match "ReparsePoint") {
                $analysis += "源目录: 已存在（是一个重解析点/联接）`n"
            } else {
                $analysis += "源目录: 已存在（是一个普通目录）`n"
            ...
            $analysis += "源目录: 无法访问（权限不足）`n"
        }
    } else {
        $analysis += "源目录: 不存在（将创建新联接点）`n"
    }
    ...
    [System.Windows.Forms.MessageBox]::Show($analysis, "目录分析", 
        [System.Windows.Forms.MessageBoxButtons]::OK, 
...
$buttonCreate.Add_Click({
    ...
    # 验证输入
    if ($source -eq "") {
        [System.Windows.Forms.MessageBox]::Show("请输入源目录", "错误", 
            [System.Windows.Forms.MessageBoxButtons]::OK, 
            [System.Windows.Forms.MessageBoxIcon]::Warning)
        $statusLabel.Text = "错误：源目录不能为空"
        return
    }
    
    if ($target -eq "") {
        [System.Windows.Forms.MessageBox]::Show("请输入目标目录", "错误", 
            [System.Windows.Forms.MessageBoxButtons]::OK, 
            [System.Windows.Forms.MessageBoxIcon]::Warning)
    ...
    if (-not (Test-Path $target)) {
        $result = [System.Windows.Forms.MessageBox]::Show("目标目录不存在，是否创建？", "确认", 
        ...
        if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("无法创建目标目录：$_", "错误", 
                    [System.Windows.Forms.MessageBoxButtons]::OK, 
    ...
    if ($checkBoxBackup.Checked -and (Test-Path $target)) {
        ...
        if ($targetItems.Count -gt 0) {
            ...
            try {
                ...
                $result = [System.Windows.Forms.MessageBox]::Show("无法创建备份：$_`n是否继续？", "备份失败", 
                    [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                    [System.Windows.Forms.MessageBoxIcon]::Warning)
    ...
    if (Test-Path $source) {
        ...
        if ($attr -match "ReparsePoint") {
            # 如果是重解析点，询问是否删除
            $result = [System.Windows.Forms.MessageBox]::Show("源目录是一个现有的联接点。是否删除并创建新的联接？", "确认", 
                [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                [System.Windows.Forms.MessageBoxIcon]::Question)
            ...
            if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                try {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("无法删除现有重解析点：$_", "错误", 
                        [System.Windows.Forms.MessageBoxButtons]::OK, 
                        [System.Windows.Forms.MessageBoxIcon]::Error)
            ...
            if ($fileCount -gt 0) {
                if ($checkBoxCopyContent.Checked) {
                    ...
                    $result = [System.Windows.Forms.MessageBox]::Show("源目录包含 $fileCount 个文件（约 $sizeMB MB）。`n是否复制这些文件到目标目录，然后删除源目录并创建联接？", "确认复制", 
                        [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                        [System.Windows.Forms.MessageBoxIcon]::Question)
                    ...
                    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                        ...
                        if ($copyResult) {
                            ...
                            if ($checkBoxVerifyCopy.Checked) {
                                ...
                                if ($verifyResult -notlike "验证成功*") {
                                    $result = [System.Windows.Forms.MessageBox]::Show("$verifyResult`n`n是否继续删除源目录并创建联接？", "验证警告", 
                                        [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                                        [System.Windows.Forms.MessageBoxIcon]::Warning)
                            ...
                            try {
                                ...
                                [System.Windows.Forms.MessageBox]::Show("无法删除源目录：$_`n但文件已复制到目标目录。", "警告", 
                                    [System.Windows.Forms.MessageBoxButtons]::OK, 
                                    [System.Windows.Forms.MessageBoxIcon]::Warning)
                        ...
                        # 用户选择不复制
                        $result = [System.Windows.Forms.MessageBox]::Show("是否删除源目录并创建联接（不复制内容）？", "确认", 
                            [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                            [System.Windows.Forms.MessageBoxIcon]::Question)
                        ...
                        if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                            try {
                                ...
                                [System.Windows.Forms.MessageBox]::Show("无法删除源目录：$_`n目录可能正在被使用。", "错误", 
                                    [System.Windows.Forms.MessageBoxButtons]::OK, 
                                    [System.Windows.Forms.MessageBoxIcon]::Error)
                    ...
                    # 自动复制选项未启用
                    $result = [System.Windows.Forms.MessageBox]::Show("源目录存在且包含文件，但'自动复制'选项未启用。`n是否启用此选项并继续？", "选项建议", 
                        [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                        [System.Windows.Forms.MessageBoxIcon]::Question)
                ...
                # 源目录存在但为空
                $result = [System.Windows.Forms.MessageBox]::Show("源目录存在但为空。是否删除并创建联接？", "确认", 
                    [System.Windows.Forms.MessageBoxButtons]::YesNo, 
                    [System.Windows.Forms.MessageBoxIcon]::Question)
                ...
                if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                    try {
                        ...
                        [System.Windows.Forms.MessageBox]::Show("无法删除源目录：$_", "错误", 
                            [System.Windows.Forms.MessageBoxButtons]::OK, 
                            [System.Windows.Forms.MessageBoxIcon]::Error)
                        $statusLabel.Text = "错误：无法删除源目录"
                        return
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            [System.Windows.Forms.MessageBox]::Show($message, "成功", 
                [System.Windows.Forms.MessageBoxButtons]::OK, 
                [System.Windows.Forms.MessageBoxIcon]::Information)
            ...
            [System.Windows.Forms.MessageBox]::Show($errorMsg, "错误", 
                [System.Windows.Forms.MessageBoxButtons]::OK, 
                [System.Windows.Forms.MessageBoxIcon]::Error)
        }
    } catch {
        ...
        [System.Windows.Forms.MessageBox]::Show("创建目录联接时发生异常：$_", "错误", 
            [System.Windows.Forms.MessageBoxButtons]::OK, 
            [System.Windows.Forms.MessageBoxIcon]::Error)
    }
})
...
$form.Add_Load({
    ...
    if (-not (Test-Administrator)) {
        $result = [System.Windows.Forms.MessageBox]::Show("创建目录联接需要管理员权限。是否以管理员身份重新运行？", "需要管理员权限", 
            [System.Windows.Forms.MessageBoxButtons]::YesNo, 
            [System.Windows.Forms.MessageBoxIcon]::Warning)
